...
theorem mgf_of_gaussian (X: Ω → ℝ) (hXm: Measurable X) (hX: (Measure.map X μ) = (gaussianReal 0 1)) : ∀ t : ℝ, mgf X μ t = Real.exp (t ^ 2 / 2) := by
  intro t
  rw [ProbabilityTheory.mgf_gaussianReal hX t]
  have h: (NNReal.toReal 1) = 1 := by
    exact rfl
  rw [h]
  ring_nf
...
#check mgf_sum_of_identDistrib
#check ProbabilityTheory.mgf_smul_left
...
(hident : ∀ (i : ℕ), ∀ (j : ℕ), ProbabilityTheory.IdentDistrib (Y i) (Y j) μ μ) -- identically distributed
(Z_def : ∀ n : ℕ, Z n = (Real.sqrt n)⁻¹ • (∑ i ∈ Finset.range n, (Y i)) ) -- def of Z as the sum of a given number of random variables
: ∀ n : ℕ, n > 0 → (∀ t : ℝ, mgf (Z n) μ t = (mgf (Y 0) μ ((√n)⁻¹ * t)) ^ n) := by
  intro n hn t
  rw [Z_def]
  rw [ProbabilityTheory.mgf_smul_left]
  have h0: 0 ∈ (Finset.range n) := by
    exact Finset.mem_range.mpr hn
  have h_ident₁ : ∀ i ∈ (Finset.range n), ∀ j ∈ (Finset.range n), ProbabilityTheory.IdentDistrib (Y i) (Y j) μ μ := by
    intro i j
    exact fun j a ↦ hident i j
  rw [mgf_sum_of_identDistrib h_meas h_indep h_ident₁ h0 ((√n)⁻¹ * t)]
  have : (Finset.range n).card = n := by
